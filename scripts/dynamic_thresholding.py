...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Accordion("Dynamic Thresholding", open=False):
            gr.HTML(value=f"Enable Dynamic Threasholding (CFG Scale Fix). See the <a href=\"https://github.com/mcmonkeyprojects/sd-dynamic-thresholding/wiki/Usage-Tips\">Usage Tips wiki page.</a>", elem_id='dynthres_wiki_link')
            with gr.Row():
            ...
            with gr.Accordion("Advanced Options", open=False, elem_id='dynthres_advanced_opts'):
                ...
                with gr.Row():
                    ...
                    mimic_scale_min = gr.Slider(minimum=0.0, maximum=30.0, step=0.5, label="Minimum Mimic Scale Scheduler", elem_id='dynthres_mimic_scale_min')
                with gr.Row():
                    cfg_mode = gr.Dropdown(VALID_MODES, value="Constant", label="CFG Scale Scheduler", elem_id='dynthres_cfg_mode')
                    cfg_scale_min = gr.Slider(minimum=0.0, maximum=30.0, step=0.5, label="Minimum CFG Scale Scheduler", elem_id='dynthres_cfg_scale_min')
